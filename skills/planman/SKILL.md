...
diff of this repository (or on a markdown document). Your job is to
work them one at a time and mark each resolved as you go. The reviewer
is watching live; the page updates as you resolve and reply.
...
- Diff threads: `{"file":"path","side":"old|new","line":N,"context":"<exact line text>"}`.
  ...
  `context`, not just the line number, because the file may have
  shifted. A `start_line` field, when present, widens the anchor to the
  span from `start_line` through `line` on that side; `line` is always
  the span's end and the anchored `context` line. The anchor also
  records `base`/`head` (and resolved `base_sha`/`head_sha`), the
  comparison the reviewer was looking at when they wrote it. A head of
  `@worktree` or `@index` means uncommitted state at the time.
...
3. If you did not make the change, or made a different one than asked,
   reply in the thread explaining why. Resolve it only if the
   reviewer's question is genuinely settled:
...
1. `GET $P/api/hunks` returns the manifest of the current diff. Every
   hunk has a stable `id` (`path:hN`) plus its header, line ranges,
   add/delete counts, and first lines. Reference hunks only by these
   ids; never embed diff text.
2. Compose the walkthrough JSON and `POST $P/api/walkthrough`:
   ...
   Rules: 1-8 chapters; 1-20 stops per chapter; 1-14 `hunk_ids` per
   stop; `importance` is `critical`, `normal`, or `context`. Order
   stops as a story: the why, the core change, then the periphery. Put
   noise such as generated files and mechanical renames in `support`,
   not stops.
